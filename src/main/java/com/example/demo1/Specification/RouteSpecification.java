...
public class RouteSpecification {
    public Specification<Route> hasSource(String source){
        return (root, criteriaQuery, cb) -> source==null ? null : cb.equal(root.get("source"),source);
    }

    public Specification<Route> hasDestination(String destination){
        return (root, query, cb) ->  destination==null ? null : cb.equal(root.get("destination"),destination);
    }

    public Specification<Route> hasDepartureTime(String departureTime){
        return (root, query, cb) -> departureTime==null ? null : cb.equal(root.get("departureTime"),departureTime);
    }

    public Specification<Route> hasArrivalTime(String arrivalTime){
        return (root, query, cb) -> arrivalTime==null ? null : cb.equal(root.get("arrivalTime"),arrivalTime);
    }
